...
void contarPalabras(struct shared_use_st * );
...
int main(int argc, char *argv[]) 
{
  ...
  modificar_sem(LOCK, semid, sb); //Bloquea esperando que alguien lo habilite. Si agarra primero cuando principal hace SETVAL=1, pasa. Sino, esperará a que Ej3Letras haga UNLOCK

  contarPalabras(shared_stuff); //Lee la memoria

  modificar_sem(UNLOCK, semid, sb); //Si fue el primero, libera para que Ej3Letras lea y si fue el segundo, libera para que Ej3 borre la shm y el sem
...
void contarPalabras(struct shared_use_st * shared_stuff)
{
  int i=0;
  int cant=0;

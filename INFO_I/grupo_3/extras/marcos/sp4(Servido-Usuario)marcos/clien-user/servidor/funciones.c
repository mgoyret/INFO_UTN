...
int ingresar (Datos clienteIngresado,int clientSocketFd)
{
    ...
    if (fp!=NULL)
    {
        ...
        printf("Ahora en funcion ingresar:\nAl usuario '%s' lo guarde en variable usuario y vale '%s'\nLa contra '%s' la guarde en contra y vale '%s'\n", clienteIngresado.usuario, usuario, clienteIngresado.contra, contra);
    ...
    printf("Saliendo de funcion ingresar\n");
    ...
    *distintos=EXITO;
    ...
    {   
        ...
        for (j = 0; j < ARRA; j++)
        {
            ...
            /* A veces en el archivo se agrega el caracter "DEL" (ASCII = 127) */
            if(respaldo_datos[strlen(respaldo_datos)-1] == 127)
            {
                respaldo_datos[strlen(respaldo_datos)-1] = '\0';
            }
        ...
        printf("Ingresado:\t\t'%s' (largo %ld)\nEn Base de datos:\t'%s' (largo %ld)\nstrcmp: %d\n\n", respaldo_usuario, strlen(respaldo_usuario), respaldo_datos, strlen(respaldo_usuario), strcmp(respaldo_usuario, respaldo_datos));

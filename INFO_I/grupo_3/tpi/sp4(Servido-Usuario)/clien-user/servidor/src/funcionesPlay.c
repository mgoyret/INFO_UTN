...
int mostrar_directorios(char* nombre_dir, int clientSocketFd)
{
    ...
    char artista[30], dir_artista[60], buffer[SONG_BUF];
        #ifndef ALGO
            char DBGMSG[100]
        #endif
        #ifndef ALGO
    ...
    {
        RESALTAR
        printf("\n\nAbri carpeta con artistas\n\n");
        BLANCO
            #ifndef ALGO
                sprintf(DBGMSG,"Directorio abierto exitosamente\n");
                log_msg(INFO,__FILE__,__LINE__,DBGMSG);
            #endif
        memset(buffer, SONG_BUF, '\0');
        while ((dir_entry = readdir(dp)) != NULL)
        {
            ...
            if ( (dir_entry->d_type == DT_DIR) && (strcmp(dir_entry->d_name,".")!=0) && (strcmp(dir_entry->d_name,"..")!=0) )
            {
                i++;
                printf("Vuelta %d\n", i);
                //messageSize = send(clientSocketFd, dir_entry->d_name, strlen(dir_entry->d_name)+1, 0); /* 1 */
                //usleep(10000);
                strcat(buffer, "-");
                strcat(buffer, dir_entry->d_name);
                strcat(buffer, "\n");

                printf("Se envio %d bits\n",messageSize);
            }
        }
        send(clientSocketFd, buffer, SONG_BUF, 0); /* 1 */
        printf("Enviado buffer\n%s\n", buffer);
...
{
    char cancion[30], song_buffer[SONG_BUF];
    int i = 0, salida = EXITO, messageSize = 0;
    DIR* dp;
    char camino_cancion[60], buffer[SONG_BUF];
    #ifndef ALGO
        char DBGMSG[100];
    #endif
    struct dirent *dir_entry, *elejida = NULL;
    ...
    if (dp != NULL)
    {
        ...
        send(clientSocketFd, buffer, SONG_BUF, 0); /* 4 */
        printf("Se envio\n%s\n", buffer);
        if(i > 0)
        {
            rewinddir(dp);
            ...
            while ((dir_entry = readdir(dp)) != NULL)
            {
                if (dir_entry->d_type != DT_DIR)
                {
                    ...
                    {
                        printf("Se encontro coincidencia. Seleccionada es '%s' y en el archivo esta '%s'\n", cancion, dir_entry->d_name);
                        elejida = dir_entry;
                    }
                }
            }
            if (elejida != NULL)
            {
                ...
                //enviarCancion(clientSocketFd,cancion,dir_artista);
                ...
                fp=fopen((camino_cancion),"rb");
                if (fp!=NULL)
                {
                    memset(song_buffer, SONG_BUF, '\0');
                 	while (!feof(fp))
    		        {
	    		        fread(song_buffer, sizeof(char), SONG_BUF, fp);
                        if(!feof(fp))
    	    		        send(clientSocketFd, song_buffer, SONG_BUF, 0);/* 7 */ //ENVIANDO CANCION
        	    	}
                sleep(1);
		        send(clientSocketFd, "FIN", 4, 0);/* 7 */
    		    printf("Cancion enviada\n");    
                fclose(fp);
                }
                else
                {
                    RESALTAR
                    ROJO
                    printf("No funk fopen de cancion :(\n");
                    BLANCO
                }
